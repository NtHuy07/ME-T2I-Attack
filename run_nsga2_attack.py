...
from nsga2 import RankAndCrowdingSurvival, ElitistArchive
...
class Genetic():
    ...
    def __init__(self, ori_sent, log_save_path, intem_img_path, 
        ...
        self.selection = RankAndCrowdingSurvival()
    ...
    def get_fitness_score(self, input_tokens, gen):
        ...
        for cnt, tokens in enumerate(input_tokens):
            ...
            fitness = np.array([- image_similarity.item(), - text_similarity.item()])
            ...
            # Save individual to elitist archive
            self.archive.update(f"{str(gen)}-{str(cnt)}", fitness)
    ...
    def run(self, log=None):
        itr = 0
        old_scores = np.array([]).reshape(0, 2)
        ...
        while itr <= self.max_iters:
            ...
            self.me.update_map(new_pop, - new_scores[:, 0], new_phenos, 
                            self.max_sent, self.max_fitness, itr)
            

            # Select P (elite)
            elite_ind = self.selection.do(pop_scores, self.pop_size)
            ...
            # Save NSGA elitist archive
            self.archive.save(os.path.join(self.log_path, "archive"), itr)
